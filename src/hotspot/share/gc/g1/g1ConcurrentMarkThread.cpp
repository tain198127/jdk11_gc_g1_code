...
void G1ConcurrentMarkThread::run_service() {//这里是开始执行CM的正式入口
  ...
  while (!should_terminate()) {//等到started状态被设置了，才会启动，不然就一直sleep
    ...
    GCTraceConcTime(Info, gc) tt("Concurrent Cycle");
    {
      ...
      {
        G1ConcPhase p(G1ConcurrentPhase::SCAN_ROOT_REGIONS, this);//扫表root regions
      ...
      if (!_cm->has_aborted()) {
        ...
        _cm->rebuild_rem_set_concurrently();//重建remember set（异步操作）
      }
